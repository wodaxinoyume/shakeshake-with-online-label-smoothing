...
class OnlineLabelSmoothing(nn.Module):
    ...
    def __init__(self, alpha: float, n_classes: int, smoothing: float = 0.1):
        ...
        self.register_buffer('supervise', torch.zeros(n_classes, n_classes).cuda())  # Move to GPU
        ...
        self.register_buffer('update', torch.zeros_like(self.supervise).cuda())  # Move to GPU
        # For normalizing we need a count for each class
        self.register_buffer('idx_count', torch.zeros(n_classes).cuda())  # Move to GPU
        self.hard_loss = nn.CrossEntropyLoss().cuda()  # Move to GPU
